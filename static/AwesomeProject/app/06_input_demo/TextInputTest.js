...
export default class TextInputTest extends Component {
    ...
    _login(){
        ToastUtil.show("登陆成功");
        //使用key来保存数据，这些数据一般是全局独有的，常常需要调用
        //除非你手动移除，这些数据被永久保存，而且默认不会过期
        storage.save({
            key: 'loginState',   //不要在key中使用下划线符号
            rawData: {
    ...
    _fetchVerifyCode(){
        ...
        if(this.state.isCounting){   //为true时不能点击
            return;
        }else{
            this._startCounting();
        }
    ...
    render() {
        return (
            <Container>
                <Header style={{ backgroundColor: '#ccc', height: 48 }}>
                    <Left>
                        {/* <Button transparent onPress={() => this._onCloseClick()}> */}
                        <Button transparent>
                            <Icon name="arrow-back" style={{ color: '#333' }}></Icon>
                        </Button>
                    </Left>
                    <Body>
                ...
                <Content style={{ backgroundColor: 'white' }}>
                    <View style={{ marginTop: 60 }}>
                        <View style={TextInputStyle.view_account_input_container}>
                            <Icon name="ios-person" style={TextInputStyle.icon_account}></Icon>
                            <TextInput
                        ...
                        <View style={TextInputStyle.view_password_input_container}>
                            ...
                            <Text onPress={() => this._fetchVerifyCode()}
