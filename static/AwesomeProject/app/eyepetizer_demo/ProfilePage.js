            ...
            <View style={ProfilePageStyle.container}>
                <View style={{alignItems: 'flex-end'}}>
                    <TouchableWithoutFeedback onPress={this.props.onSettingClick}>
                        <Image source={require('../imgs/ic_menu_more.png')} style={ProfilePageStyle.btn_setting}/>
                    </TouchableWithoutFeedback>
                </View>
                <View style={ProfilePageStyle.conatiner_avater}>
                    <TouchableWithoutFeedback onPress={this.props.onAvatarClick}>
                        <Image style={ProfilePageStyle.img_avatar} source={require('../imgs/avatar.png')}/>
                    </TouchableWithoutFeedback>
                </View>
                <View style={ProfilePageStyle.container_favority_and_reply}>
                    <TouchableWithoutFeedback onPress={this.props.onFavorityClick}>
                        <View style={ProfilePageStyle.container_favority}>
                            <Image style={ProfilePageStyle.img_favority} source={require('../imgs/ic_action_favorites_grey.png')}/>
                    ...
                    <TouchableWithoutFeedback onPress={this.props.onReplyClick}>
                        <View style={ProfilePageStyle.container_reply}>
                            <Image style={ProfilePageStyle.img_reply} source={require('../imgs/ic_action_reply_grey.png')}/>
...
class MyItem extends Component{
    render(){
        return(
            <Text tag={this.props.tag} style={ProfilePageStyle.tv_myItem} onPress={this.props.onItemClick}>
                {this.props.title}
            </Text>
        )
    }
...
export default class ProfilePage extends Component{
    ...
    _toLoginPage(){
        if(this.state.isLogin){
            return ToastUtil.show("您已登录，无需重新登录！若要打开登录界面，请在设置中退出登录");
        }
        ToastUtil.show('请先登录');
        const {navigator} = this.props;
        if(navigator){
    ...
    render(){
        return(
            <View>
                ...
                >
                    <MyItem title="我的消息" onItemClick={()=>this._onItemClick(MYITEM[0])}></MyItem>
                    <MyItem title="我的关注" onItemClick={()=>this._onItemClick(MYITEM[1])}></MyItem>
                    <MyItem title="我的缓存" onItemClick={()=>this._onItemClick(MYITEM[2])}></MyItem>
                    <MyItem title="意见反馈" onItemClick={()=>this._onItemClick(MYITEM[3])}></MyItem>
                    <MyItem title="我要投稿" onItemClick={()=>this._onItemClick(MYITEM[4])}></MyItem>
                </ScrollView>
            </View>
        )
    }
